...
// slave1 --[TELL(slave_name, slave_event, params...)]-> master --[EVENT(from, callback_id, event_name, params)]-> slave2
...
// [1] array of args
// 
// NOTE: ACK should never EVER be sent to Master, this might results in a
// callback loop.
      ...
      if (message.hasOwnProperty("type") && _.isString(message.type)) {
        ...
        switch (message.type) {
          case IDENT:
            ...
            if (self.sockets.hasOwnProperty(message.slaveName)) {
            ...
            // Send that bitch
            debug("Master - Sending ACK (id: %s)", message.id);
          ...
          case TELL:
            ...
            if (!self.sockets.hasOwnProperty(slave_name)) {
            ...
            var msg = new Message(EVENT, [message.slaveName, message.id, slave_event, params]);
          ...
          case EVENTCB:
            ...
            break;
          default:
            debug("Master received unrecognized command: %s", message.type);
        }
      }
    });
...
function Slave() {
  ...
  self.master.on("data", function(message) {
    ...
    if (message.hasOwnProperty("type")) {
      switch (message.type) {
        case ACK:
          debug("Slave  - Received ACK from Master for message: %s", message.data[0]);
        ...
        case EVENT:
          ...
          debug("Slave  - Received EVENT(%s) from %s", event_name, from);
          ...
          break;
        case SHOUT:
          debug("Slave  - Received SHOUT(%s)", message.data[0]);

          self.emit.apply(self, message.data);
          break;
        case CLOSE:
          debug("Slave  - Received CLOSE");
